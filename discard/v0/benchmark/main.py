...
def main():
    ...
    worker = Worker(buffer=buffer, buffer_price=buffer_price, lr=args.lr, gamma=args.gamma, eps_clip=args.eps_clip, max_step=args.max_step,
                    num=args.worker_num,
                    device=device, zone_table_path=args.zone_table_path, model_path=args.platform_model_path,
                    price_model_path=args.price_model_path, njobs=args.njobs, intelligent_worker=intelligent_worker, probability_worker = probability_worker, bilstm = args.bilstm, dropout = args.dropout)
    ...
    while True:
        ...
        with open("train.txt", 'a') as file:
            file.write(log+"\n")
        worker.save("platform_latest.pth", "price_latest.pth")
            ...
            if args.reward_threshold > 0:
                if checkpoint[0] + actor_episode + critic_episode <= args.eval_episode and best_reward - total_reward >= args.reward_threshold:
                    ...
                    if total_reward > best_reward:
                        ...
                        worker.save("platform_best.pth", "price_best.pth")
                    else:
                        best_epoch += 1
