...
class Worker():
    ...
    def gen_mask(self, epsilon = 0.0):
        if self.worker_mode == 1:
            self.mask = torch.zeros([self.num],dtype=torch.int).to(self.device)
            self.p = torch.zeros([self.num, 3],dtype=torch.int).to(self.device)
            self.p[:,0] = 1
            self.mask_rate = self.p[:,1]
            self.strike_rate = self.p[:,2]
        elif self.worker_mode == 2:
            self.mask = torch.zeros([self.num],dtype=torch.int).to(self.device) + 1
            self.p = torch.zeros([self.num, 3],dtype=torch.int).to(self.device)
            self.p[:,1] = 1
            self.mask_rate = self.p[:,1]
            self.strike_rate = self.p[:,2]
