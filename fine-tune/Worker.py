...
import torch
import pandas as pd
from models import Q_Net,Worker_Q_Net,Price_Net,MLP
...
def norm_mask(x, lat_min = 22.24370366972477, lat_max = 22.505171559633027, lon_min = 113.93901100917432, lon_max = 114.26928623853212, max_seat = 3):
    lat_range = lat_max - lat_min
    lon_range = lon_max - lon_min
    ...
    x[:,0] = (x[:,0] - lat_min) / lat_range
    x[:,1] = (x[:,1] - lon_min) / lon_range
    x[:,3] = x[:,3] / max_seat
...
'''
class Worker():
    def __init__(self, buffer, buffer_price, buffer_mask, lr=0.0001, gamma=0.99, eps_clip=0.2, max_step=60, history_num=5, num=1000, reservation_value=None, speed=None, capacity=None, group=None, device=None, zone_table_path = "../data/zone_table.csv", model_path = None,  price_model_path = None, mask_model_path = None, njobs = 24, intelligent_worker = False, probability_worker = False, bilstm = False, dropout = 0.0):
        super().__init__()
        ...
        self.load(model_path,price_model_path,mask_model_path,self.device)
        ...
        self.optim_mask = torch.optim.Adam(self.Mask_Net.parameters(), lr=lr, weight_decay=0.0)
        self.schedule_mask = torch.optim.lr_scheduler.ExponentialLR(self.optim_mask, gamma=0.99)

        # self.reset(max_step,num,reservation_value, speed, capacity, group)

        self.njobs = njobs

    def reset(self, max_step=60, num=1000, reservation_value=None, speed=None, capacity=None, group=None, train=True, PG_epsilon=0.0):
        self.time = 0
    ...
    def observe(self, order, current_time, exploration_rate=0):
        ...
        q_value = self.Q_training(x1, x2, x3, torch.from_numpy(self.current_order_num).to(self.device),self.mask)
        price_mu, price_sigma = self.Price_training(x1, x2, x3, torch.from_numpy(self.current_order_num).to(self.device),self.mask)
    ...
    def observe_s1(self, order, current_time, exploration_rate=0):
        self.time = current_time
        t = np.array([[self.time / 60]]*self.num)
        # self.Q_training.eval()
        torch.set_grad_enabled(False)
        ...
        q_value = self.Q_training(x1, x2, x3, torch.from_numpy(self.current_order_num).to(self.device),mask)
        price_mu, price_sigma = self.Price_training(x1, x2, x3, torch.from_numpy(self.current_order_num).to(self.device),mask)
        exploration_matrix = torch.rand_like(q_value)
        q_value[exploration_matrix<exploration_rate] = INF
        # 4. delete the Q value of not available workers
        q_value[self.observe_space[:,3]!=0] = -INF
    ...
    def observe_s2(self, order, current_time, assignment1, exploration_rate=0):
        self.time = current_time
        t = np.array([[self.time / 60]]*self.num)
        # self.Q_training.eval()
        torch.set_grad_enabled(False)
        ...
        # 4. delete the Q value of not available workers
        q_value[self.observe_space[:,3]!=0] = -INF
        # 5. only save opt-in workers
        q_value[self.mask!=0] = -INF
        # 6. delete assigned orders
        for i in range(self.num):
            if self.mask[i] == 1:
                index = assignment1[i]
                if index is not None:
                    q_value[:,int(index)] = -INF
        return q_value.cpu().detach().numpy(), price_mu.cpu().detach().numpy(), price_sigma.cpu().detach().numpy(), order_state, worker_state

    def combine(self,q_value1, price_mu1, price_sigma1, assignment1, q_value2, price_mu2, price_sigma2, assignment2):
        mask = self.mask.cpu().unsqueeze(-1).numpy()
        mask[mask>0] = 1
        q_value = q_value1 * mask + q_value2 * (1-mask)
        price_mu = price_mu1 * mask + price_mu2 * (1-mask)
        price_sigma = price_sigma1 * mask + price_sigma2 * (1-mask)


        assignment = np.array(assignment1)
        mask = mask.reshape(-1)
        assignment[mask == 0] = None
        for i in range(self.num):
            if self.mask[i] == 0:
                index = assignment2[i]
        ...
        return q_value, price_mu, price_sigma, assignment
    ...
    def save(self, path1, path2, path3):
        torch.save(self.Q_training.state_dict(), path1)
        torch.save(self.Price_training.state_dict(), path2)
        torch.save(self.Mask_Net.state_dict(), path3)


    def load(self, path1 = None, path2 = None, path3 = None, device = torch.device("cpu")):
        if device == torch.device("cpu"):
            ...
            if path3 is not None:
                self.Mask_Net.load_state_dict(torch.load(path3,map_location=torch.device('cpu')))
        else:
            if path1 is not None:
            ...
            if path3 is not None:
                self.Mask_Net.load_state_dict(torch.load(path3))
    ...
    def train_actor(self,episode,batch_size=512,train_times=10,update_critic=False,lamada=0.9,kl_threshold=0.05):
        ...
        current_state_value_target = self.Q_training(x1, x2, x3, order_num, mask)
        current_state_value_target = torch.diag(current_state_value_target).detach()
        mu_old, sigma_old = self.Price_training(x1, x2, x3, order_num, mask)
        ...
        next_state_value_target = self.Q_training(x1, x2, x3, order_num_next, mask)
        next_state_value_target = torch.diag(next_state_value_target).detach()

        next_state_value_target2 = self.Q_target(x1, x2, x3, order_num_next, mask)
        ...
        for m in pbar:
            ...
            if m!=0:
                x1, x2, x3 = norm(new_order_state, worker_state, order_state)
                mu_new, sigma_new = self.Price_training(x1, x2, x3, order_num, mask)
                ...
                if update_critic:
                    current_state_value = self.Q_training(x1, x2, x3, order_num, mask)
                    current_state_value = torch.diag(current_state_value).detach()
                    x1, x2, x3 = norm(new_order_state_next, worker_state_next, order_state_next)
                    next_state_value = self.Q_training(x1, x2, x3, order_num_next, mask)
            ...
            for _ in range(1):
                ...
                worker_state_temp, order_state_temp, order_num_temp, new_order_state_temp, \
                    ...
                    worker_state[indices], order_state[indices], order_num[indices], new_order_state[indices], \
                        ...
                        reservation_value[indices], price_next[indices], worker_action[indices], worker_reward[indices], workload_current[indices], workload_next[indices], td_target[indices], advantage[indices], mu_old[indices], sigma_old[indices], mask[indices]


                x1, x2, x3 = norm(new_order_state_temp,worker_state_temp,order_state_temp)
                current_state_value = self.Q_training(x1,x2,x3,order_num_temp,mask_temp)
                price_mu, price_sigma = self.Price_training(x1,x2,x3,order_num_temp,mask_temp)
    ...
    def train_critic(self, batch_size=512, train_times=30, show_pbar = False):
        ...
        for _ in pbar:
            worker_state, order_state, order_num, new_order_state, \
                price_old, price_log_prob_old, reward, delta_t, \
                worker_state_next, order_state_next, order_num_next, new_order_state_next, \
                reservation_value, price_next, worker_action, worker_reward, workload_current, workload_next, mask = self.buffer.sampling(batch_size, self.device)
            ...
            x1, x2, x3 = norm(new_order_state_next, worker_state_next, order_state_next)
            next_state_value1 = self.Q_target(x1, x2, x3, order_num_next, mask)
            next_state_value2 = self.Q_training(x1, x2, x3, order_num_next, mask)
    ...
    def train_mask(self, batch_size=512, train_times=10,kl_threshold=0.05):
        ...
        for _ in pbar:
            ...
            loss.backward()
            # torch.nn.utils.clip_grad_norm_(self.Mask_Net.parameters(), 1.0)  # avoid gradient explosion
            has_nan = False
            for name, param in self.Mask_Net.named_parameters():
                if param.grad is not None:
                    if torch.isnan(param.grad).any():
                        has_nan = True
                        break
