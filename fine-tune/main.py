...
def get_args():
    ...
    parser.add_argument('--worker_mode', type=int, default=0) # 0: GDPR, 1: all opt-in, 2: all opt-out
    ...
    parser.add_argument('--lowest_utility', type=float, default=35) # low: 35, high: 70
    parser.add_argument('--PG_epsilon', type=float, default=0.03)

    parser.add_argument("--platform_model_path",type=str,default="../pre-train/0.2/platform_latest.pth")
    parser.add_argument("--price_model_path",type=str,default="../pre-train/0.2/price_latest.pth")
...
def main():
    ...
    worker = Worker(buffer=buffer, buffer_price=buffer_price, buffer_mask=buffer_mask, lr=args.lr, gamma=args.gamma, eps_clip=args.eps_clip, max_step=args.max_step,
                    ...
                    price_model_path=args.price_model_path, mask_model_path = args.mask_model_path, njobs=args.njobs, intelligent_worker=intelligent_worker, probability_worker = probability_worker, bilstm = args.bilstm, dropout = args.dropout, worker_mode = worker_mode)
    ...
    while True:
        ...
        worker.reset(max_step=args.max_step, num=args.worker_num, reservation_value=reservation_value,
                     speed=speed,
                     capacity=capacity, group=group, train=True, PG_epsilon = args.PG_epsilon)
        ...
        confirmation_time = np.mean(platform.confirmation_time)

        log = "Train Episode {:} , Platform Reward {:} , Worker Reward {:} , Order Pickup {:} , Worker Reject Num {:} , Average Detour {:} , Average Travel Time {:} , Total Timeout Order {:} , Total Valid Distance {:} , Critic Loss {:} , Actor Loss {:} , Worker Loss {:} , Mask Rate {:} , Strike Rate {:} , Average Confirmation Time {:}".format(
            j, total_reward, worker_reward, total_pickup, worker_reject, average_detour, average_travel_time, total_timeout, total_valid_distance, c_loss, a_loss, w_loss, mask_rate, strike_rate, confirmation_time)
        print(log)
        ...
        if j % args.eval_episode == 0:
            ...
            log = "Eval Episode {:} , Platform Reward {:} , Worker Reward {:} , Order Pickup {:} , Worker Reject Num {:} , Average Detour {:} , Average Travel Time {:} , Total Timeout Order {:} , Total Valid Distance {:}, Mask Rate {:} , Strike Rate {:} , Average Confirmation Time {:}".format(
                j, total_reward, worker_reward, total_pickup, worker_reject, average_detour, average_travel_time, total_timeout, total_valid_distance, mask_rate, strike_rate , confirmation_time
            )
            print(log)
            ...
            dic = {
                ...
                'mask': worker.mask,
                'mask_rate': worker.mask_rate,
                'strike_rate': worker.strike_rate
            }
