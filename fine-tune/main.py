...
def get_args():
    ...
    parser.add_argument('--lowest_utility', type=float, default=35)
    parser.add_argument('--PG_epsilon', type=float, default=0.03)

    parser.add_argument("--platform_model_path",type=str,default="../pre-train/0.2/platform_best.pth")
    parser.add_argument("--price_model_path",type=str,default="../pre-train/0.2/price_best.pth")
...
def main():
    ...
    while True:
        ...
        if critic_phase:  # train critic
            ...
            pbar = tqdm.tqdm(range(args.max_step))
            for t in pbar:
                q_value1, price_mu1, price_sigma1, order_state, worker_state = worker.observe_s1(demand.current_demand, t,
                                                                                           exploration_rate_temp)
                assignment1, _ = platform.assign(q_value1)
                q_value2, price_mu2, price_sigma2, _, _ = worker.observe_s2(order_state, t, assignment1,
                                                                                           exploration_rate_temp)
                assignment2, _ = platform.assign(q_value2)
                q_value, price_mu, price_sigma, assignment = worker.combine(q_value1, price_mu1, price_sigma1, assignment1, q_value2, price_mu2, price_sigma2, assignment2)
